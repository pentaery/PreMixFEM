...
PetscErrorCode create_fracture_kappa(PCCtx *s_ctx, PetscInt cr,
                                     PetscScalar ***data) {
...
int main(int argc, char **argv) {
  PetscCall(SlepcInitialize(&argc, &argv, (char *)0,
                            "This is a code for testing adaptivity on the "
  ...
  PetscCall(PetscViewerHDF5Open(PETSC_COMM_SELF,
                                "data/fracture256/fracture256_4.h5",
                                FILE_MODE_READ, &reader));
  ...
  PetscCall(VecGetArray3d(frac_kappa, SAMPLE_LEN, SAMPLE_LEN, SAMPLE_LEN, 0, 0,
                          0, &arr_frac_kappa));
  PetscCall(create_fracture_kappa(&s_ctx, cr, arr_frac_kappa));
  PetscCall(VecRestoreArray3d(frac_kappa, SAMPLE_LEN, SAMPLE_LEN, SAMPLE_LEN, 0,
                              0, 0, &arr_frac_kappa));
